import { type DocSection, type DocTreeNode, isDocSection } from '@/utils'
        ...
        <div className="border px-2 py-1 rounded-lg">
          ...
          >
            <div className="bg-primary text-secondary flex aspect-square size-8 items-center justify-center rounded-lg">
              <Icon icon={props.current.data.icon ?? 'lucide:book-open'} width={16} height={16} />
            </div>
            <div className="grid flex-1 text-left text-secondary-foreground text-sm leading-tight">
              <span className="truncate font-medium">{props.current.data.label}</span>
              <span className="truncate text-xs opacity-60">{props.current.data.description}</span>
            </div>
            <Icon
