      ...
      <NavigationMenuLink asChild>
        ...
        >
          <div className="text-sm font-medium leading-none flex items-center gap-1">
            {icon && <Icon icon={icon} width={20} height={20} className="mr-2" />}
            {title}
          </div>
      ...
      <div className="sticky top-0 z-50 w-full p-2 py-3 border-b bg-background/70 backdrop-blur-sm">
        <div className="max-w-[var(--container-width)] mx-auto flex items-center justify-between">
          <a href="/" className="flex sm:hidden items-center space-x-2">
            <span className="font-bold text-primary text-lg">💧 {config.projectName}</span>
          </a>
          ...
          <div className="hidden md:flex items-center justify-between w-full">
            ...
            <SearchCommand items={props.searchableDoc} />
                        ...
                        <>
                          <NavigationMenuTrigger className="flex items-center">
                            {'icon' in element && element.icon && (
                              <Icon icon={element.icon} width={16} height={16} className="mr-2" />
                            )}
                            {element.label}
                          </NavigationMenuTrigger>
                          <NavigationMenuContent>
          ...
          <div className="flex items-center gap-2 sm:hidden">
            ...
            <Sheet>
              ...
              <SheetContent className="flex flex-col justify-between gap-4" side="left">
                <div>
                  <SheetHeader>
                        ...
                        <div key={index}>
                          ...
                          <div>
                            <ul className="grid w-[400px] gap-2 p-1 md:w-[500px] md:grid-cols-2 lg:w-[600px]">
                              {element.children.map((item) => (
                                <a key={item.id} href={`/docs/${item.id}`} className="text-sm">
                                  {item.data.title}
                                </a>
                              ))}
                            </ul>
