...
export default function CodeGroupComponent(props: PropsWithChildren<CodeGroupProps>) {
  const icons = config.content.icons
  const [activeTab, setActiveTab] = useState(0)
  const [tabs, setTabs] = useState<{ label: string; language: string; icon: string }[]>([])
  ...
  }, [props.children, isPropsMode, tabs])
    ...
    return (
      <div className="code-group border rounded-md overflow-hidden mb-5 bg-background">
        ...
        <div className="code-group-content">
          <div dangerouslySetInnerHTML={{ __html: parsedCodes[activeTab] || '' }} />
        </div>
      </div>
    )
  }
